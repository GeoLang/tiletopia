...
fn ecef_to_geodetic(x: f64, y: f64, z: f64) -> (f64, f64, f64) {
    ...
    let lat = (z + ep2 * b * theta.sin().powi(3)).atan2(p - WGS84_E2 * WGS84_A * theta.cos().powi(3));
    let sin_lat = lat.sin();
    let n = WGS84_A / (1.0 - WGS84_E2 * sin_lat * sin_lat).sqrt();
    let h = p / lat.cos() - n;
...
fn latlon_to_utm(lat_deg: f64, lon_deg: f64, zone: u32, _north: bool) -> Result<(f64, f64), ReprojError> {
    ...
    let m = WGS84_A * ((1.0 - WGS84_E2 / 4.0 - 3.0 * WGS84_E2.powi(2) / 64.0) * lat
        - (3.0 * WGS84_E2 / 8.0 + 3.0 * WGS84_E2.powi(2) / 32.0) * (2.0 * lat).sin()
        + (15.0 * WGS84_E2.powi(2) / 256.0) * (4.0 * lat).sin());
    let _ = e; // used for clarity

    let easting = k0 * n_val * (a_val + (1.0 - t + c) * a_val.powi(3) / 6.0
        + (5.0 - 18.0 * t + t * t) * a_val.powi(5) / 120.0) + 500_000.0;
    let mut northing = k0 * (m + n_val * lat.tan() * (a_val.powi(2) / 2.0
...
fn utm_to_latlon(easting: f64, northing: f64, zone: u32, north: bool) -> Result<(f64, f64), ReprojError> {
    let k0 = 0.9996;
    ...
    let lat = phi1 - (n1 * phi1.tan() / r1) * (d.powi(2) / 2.0
        ...
        + (61.0 + 90.0 * t1 + 298.0 * c1 + 45.0 * t1 * t1 - 252.0 * e_prime_sq) * d.powi(6) / 720.0);
    let lon0 = ((zone as f64 - 1.0) * 6.0 - 180.0 + 3.0) * PI / 180.0;
...
mod tests {
    ...
    fn test_identity_transform() {
        ...
        let c = Coord3D { x: 10.0, y: 48.0, z: 100.0 };
    ...
    fn test_wgs84_to_utm() {
        ...
        let c = Coord3D { x: 9.0, y: 48.0, z: 0.0 }; // lon=9, lat=48
        let r = t.transform(c).unwrap();
    ...
    fn test_roundtrip_utm() {
        ...
        let c = Coord3D { x: 11.5, y: 47.3, z: 500.0 };
    ...
    fn test_wgs84_to_ecef() {
        ...
        let c = Coord3D { x: 0.0, y: 0.0, z: 0.0 }; // null island
        let r = t.transform(c).unwrap();
